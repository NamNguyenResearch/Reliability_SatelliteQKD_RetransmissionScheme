function [jointProbabiliteGamma]=jointProbabilite_SIM_Gamma(bit_Alice,bit_Bob,ScaleCo,P_T_dBm,C2n) 
    ...
    EI0=1/(4*FSL)*M*R*G_Tx*P_T*h_p_G*ModDepth*hl*G_Rx;
    EI1=-EI0;
    ...
    for i=1:length(t)
        a(i)=teta(i)/temp1;     
        for l=1:length(t)
            I0=1/(4*special(i)*FSL)*G_Tx*M*R*P_T*h_p_G*ModDepth*hl*t(l)*G_Rx;
            I1=-I0;
            
            sigmaN_i_l=sqrt(2*q*M^(2+x)*(R*G_Tx*1/(4*special(i)*FSL)*P_T*h_p_G*ModDepth*hl*t(l)*G_Rx+Id)*deltaf+4*kB*T/RL*deltaf);
