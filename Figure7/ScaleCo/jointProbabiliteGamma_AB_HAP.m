function jointProbabiliteGamma=jointProbabiliteGamma_AB_HAP(bit_Alice,bit_Bob,ScaleCo)
    ...
    integral=quad(@calculateSigma_R_2_Tempt,h0,H_P);
    ...
    for i=1:length(t)
        ...
        for l=1:length(t)
            ...
            sigmaN_i_l=sqrt(2.*q.*FA.*(M.^2).*R.*(1./(4.*special(i)).*R.*M.*P_r_G.*ModDepth.*t(l)+1./special(i).*P_b_P.*G_RX_P.*...
                       G_A.*G_TX_P.*h_p_G.*G_RX_G.*hl.*t(l)+1./special(i).*P_a.*G_RX_P.*G_A.*G_TX_P.*h_p_G.*G_RX_G.*hl.*t(l)+...
