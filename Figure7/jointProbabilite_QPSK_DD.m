function jointProbabiliteGamma=jointProbabilite_QPSK_DD(bit_Alice,bit_Bob,ScaleCo,P_T_dBm,C2n) 
    ...
    EI0=0.25*M*R*G_Tx*1/FSL*P_T*h_p_G*hl*G_Rx;
    EI1=-EI0;
    ...
    for i=1:length(t)
        a(i)=teta(i)/temp1;     
        for l=1:length(t)
            I0=0.25*M*R*G_Tx*1/FSL*P_T*hl*h_p_G*t(l)/special(i)*G_Rx;
            I1=-I0;
            
            sigmaN_i_l=sqrt(2*q*M^(2+x)*(1*R+Id)*deltaf+4*kB*T/RL*deltaf);
