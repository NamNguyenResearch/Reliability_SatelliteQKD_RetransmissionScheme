function jointProbabiliteGamma=jointProbabilite_QPSK_Gamma(bit_Alice,bit_Bob,ScaleCo,P_T_dBm,C2n,Attenuation) 
    ...
    v=[0.308441115765,0.401119929155,0.218068287612,0.0620874560987,0.00950151697518,...
       0.000753008388588,0.0000282592334960,0.000000424931398496,0.00000000183956482398,...
       0.000000000000991182721961];    
    ...
    G_Tx=10^(G_Tx_dB/10);
    G_Rx_dB=131;                   %Rx telescope gain(dB)
    G_Rx=10^(G_Rx_dB/10);
    ...
    EI0=M*R*sqrt(G_Tx*1/FSL*P_T*h_p_G*hl*G_Rx*P_LO);
    EI1=-EI0;
    ...
    for i=1:length(t)
        a(i)=teta(i)/temp1;     
        for l=1:length(t)
            I0=M*R*sqrt(G_Tx*1/FSL*P_T*h_p_G*hl*t(l)/special(i)*G_Rx*P_LO);
            I1=-I0;
            
            sigmaN_i_l=sqrt(2*q*M^(2+x)*(R*P_LO+R*G_Tx*1/FSL*P_T*h_p_G*hl*t(l)/special(i)*G_Rx+Id)*deltaf+4*kB*T/RL*deltaf);
